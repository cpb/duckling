...
#
# Every one of these edges was originally pinned by a real zone, and every one
# of them was hostage to something outside this repo. Europe/Dublin is only a
# negative-DST zone where tzdata is compiled in vanguard format — on a
# rearguard host (Debian, Ubuntu) the test kept passing and stopped
# distinguishing anything. America/Nuuk grew its late-in-the-day gap in tzdata
# 2023a and answered to a different name before 2020a. Australia/Lord_Howe is
# the last zone in use with a 30-minute gap, so that coverage rests on one
...
# world behaves as expected where the database can say — but they are what
# makes the behavior itself permanently covered, on the legs where the real
# zones go quiet.
#
