...
module Duckling
  ...
  #
  # Neither datasource exposes a version: `RubyDataSource`'s `version_info`
  # is private, and `ZoneinfoDataSource` offers only `zoneinfo_dir`. So
  ...
  module TZInfoCapabilities
    ...
    # DataSourceNotFound is rescued alongside InvalidTimezoneIdentifier, and it
    # is a *sibling* class rather than a subclass, so it has to be named. The
    # two mean different things — "this database doesn't know that zone" vs
    # "there is no database" — but a probe's contract with its callers is a
    # total boolean, and a host with no tz database plainly does not have the
    # links. Letting absence escape instead turned every caller into a crash
    # site on the one configuration this gem newly supports.
    ...
    # A host with no database at all gets its own answer rather than an
    # exception. This is called to *build failure messages* — from
    # `timezone_for`'s diagnosis and from every test/environments/ contract —
    # so raising here replaces the explanation with a raw tzinfo error at
    # exactly the moment the explanation was wanted. An environment contract
    # asking "is tzdata installed in the image?" could not print that question
    # when tzdata was missing.
