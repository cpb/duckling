...
- **API shape and `reference_time:` behavior**: `test/duckling_test.rb` (parse result/interval shape, `reference_time:` coercion and type-checking — see "Public and internal APIs" below).
...
- **Native-extension infrastructure sanity**: `test/duckling_ci_test.rb` (build config files exist and are shaped correctly).
