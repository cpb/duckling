...
- **`bin/setup`** — `bundle install`, then (if `.env.local` doesn't already exist) copies `.env.local.example` to `.env.local`, which sets `RB_SYS_CARGO_PROFILE=dev`. Run this first in a fresh checkout/worktree.
