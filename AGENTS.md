...
- **`bin/setup`** — runs `brew bundle` (installing the Rust toolchain per `Brewfile`) when Homebrew is present, then `bundle install`, then (if `.env.local` doesn't already exist, and `CI` isn't `true`) copies `.env.local.example` to `.env.local`, which sets `RB_SYS_CARGO_PROFILE=dev`. No-ops the Homebrew step gracefully on machines without `brew`, and skips the `.env.local` seed entirely in CI (which always wants the release profile) — see below. Run this first in a fresh checkout/worktree.
